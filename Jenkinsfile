pipeline {
    ...
    stages {        
        ...
        stage('Stop previous test instance') { 
            steps {
                sh '''docker stop jenkins-assignment-test
                    sudo docker container ls -a | grep jenkins-assignment-test | cut -c -12 > ~/docker_jenkins_no_pipeline_test.txt
                    cat ~/docker_jenkins_no_pipeline_test.txt | xargs sudo docker container rm -f'''
            }
        }
        stage('Deploy to test') { 
            steps {
                sh 'docker run --name jenkins-assignment-test -d -p 8084:80 nginx-server:v2'
            }
        }
        stage('Smoke test') { 
            steps {
